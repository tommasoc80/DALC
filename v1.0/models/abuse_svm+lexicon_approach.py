...
def validate(Xtrain,Ytrain,task_type):
    ...
    text_features = FeatureUnion([('word', count_word),
                                  ('char', count_char),
                                  ('length', features.TweetLength()),
                                  ('badwords', features.Lexicon('groflex.tsv'))
                                  ])
...
def train(Xtrain, Ytrain, x_dev, y_gold):

    # Train and test on dev/test and evaluate
    ...
    Yguess = pipeline_svm.predict(x_dev)
...
def training_data(inputtr,inputdev,task_type):
    ...
    #validate(Xtrain,Ytrain,task_type) # for grid search
    train(Xtrain, Ytrain, x_dev,y_gold) # to test models on dev and test data
...
if __name__ == '__main__':
    ...
    training_set = sys.argv[1] # training data
